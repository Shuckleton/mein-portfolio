...
const RetroPC = () => {
    ...
    cloneScene.traverse((obj) => {
      if (obj.isMesh) {
        if (obj.name === 'MonitorScreen') {
          ...
          clonedMonitor.material = obj.material.clone(); // ensure separate material
        ...
        if (obj.material?.name === 'Monitor_Screen') {
          const videoMaterial = new THREE.ShaderMaterial({
            uniforms: {
              ...
              saturation: { value: 1.1 }, // 1.0 = normal, 2.0 = double saturation
            },
            vertexShader: `
              varying vec2 vUv;
              void main() {
  ...
  return (
    <>
      ...
      <pointLight intensity={3} position={[0, 10, 10]} color="#ffffff" />
      <pointLight intensity={1.5} position={[-10, 5, 0]} color="#ffeedd" />
      <pointLight intensity={1.2} position={[10, -5, -10]} color="#aaaaff" />
      <spotLight
        position={[15, 40, 20]}
        angle={0.2}
        penumbra={0.7}
